...
This repo follows the same planning convention as its siblings (`livepeer-payment-library`, `openai-livepeer-bridge`). The sections below are intentionally mirror-copies; differences are called out explicitly.
...
IDs are monotonic, zero-padded to 4 digits, unique within this repo. Cross-repo coordination (e.g. this repo's `0001` depending on `livepeer-payment-library`'s `0018`) is captured in the `## Cross-repo dependencies` section of a plan.
