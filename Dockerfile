...
# Pre-fetch deps before copying full source so the layer cache stays
# hot when only Go source changes.
COPY go.mod go.sum ./
RUN go mod download
...
RUN CGO_ENABLED=0 GOOS=linux go build \
        ...
        -o /out/openai-worker-node \
        ./cmd/openai-worker-node
...
COPY --from=builder /out/openai-worker-node /usr/local/bin/openai-worker-node
...
ENTRYPOINT ["/usr/local/bin/openai-worker-node"]
